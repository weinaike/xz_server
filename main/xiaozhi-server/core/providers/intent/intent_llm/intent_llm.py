...
class IntentProvider(IntentProviderBase):
    ...
    def get_intent_system_prompt(self, functions_list: str) -> str:
        ...
        prompt = (
            ...
            f"{functions_desc}\n"
            "处理步骤:\n"
            "1. 分析用户输入，用户输入的文字（来源于语音识别），因为语音识别过程中，可能存在同音字、发音不准、环境噪音等问题导致识别不准。需要根据上下文，尽量纠正这些错误。\n"
            "2. 从可用函数列表中选择最匹配的函数\n"
            "3. 如果找到匹配的函数，生成对应的function_call 格式\n"
            '4. 如果没有找到匹配的函数，返回{"function_call": {"name": "continue_chat", "true_content":"Corrected Content"}}\n\n'
            "返回格式要求：\n"
            "1. 必须返回纯JSON格式\n"
            ...
            "```\n"
            "用户: 你好啊\n"
            '返回: {"function_call": {"name": "continue_chat", "true_content":"你好啊"}}\n'
            "```\n\n"
            "注意：\n"
            "1. 只返回JSON格式，不要包含任何其他文字\n"
            '2. 如果没有找到匹配的函数，返回{"function_call": {"name": "continue_chat", "true_content":"(纠正后的内容)"}}, (注意true_content仅纠正文字,不要添加内容)\n'
    ...
    async def corrected_intent(self, conn:ConnectionHandler, dialogue_history: List[Dict],  text: str) -> str:
        if not self.llm:
            raise ValueError("LLM provider not set")
